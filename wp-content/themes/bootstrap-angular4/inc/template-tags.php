...
if (!function_exists('bootstrapBasicCategoriesList')) {
	...
	{
		return sprintf('<span class="categories-icon glyphicon glyphicon-th-list" title="' . __('Posted in', 'bootstrap-angular4') . '"></span> %1$s', $categories_list);
		...
		if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
			...
			echo '<div class="comment-body media">';
				echo '<div class="media-body">';
					_e('Pingback:', 'bootstrap-angular4');
					comment_author_link(); 
					edit_comment_link(__('Edit', 'bootstrap-angular4'), '<span class="edit-link">', '</span>');
				echo '</div>';
			echo '</div>';
		} else {
			echo '<li id="comment-';
				...
				echo '<div class="comment-content media-body">';
					echo '<div class="comment-author vcard">';
						...
						echo '<time datetime="';
							comment_time('c');
						echo '">';
						printf(_x('%1$s at %2$s', '1: date, 2: time', 'bootstrap-angular4'), get_comment_date(), get_comment_time());
						echo '</time>';
						echo '</a>';
						// end date-time
						...
						edit_comment_link('<span class="fa fa-pencil-square-o "></span>' . __('Edit', 'bootstrap-angular4'), '<span class="edit-link">', '</span>');
						...
						if ('0' == $comment->comment_approved) {
							echo '<div class="comment-awaiting-moderation text-warning"> <span class="glyphicon glyphicon-info-sign"></span> ';
								_e('Your comment is awaiting moderation.', 'bootstrap-angular4');
							echo '</div>';
						} //endif;

						// comment author says
						printf(__('%s <span class="says">says:</span>', 'bootstrap-angular4'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link()));
					...
					comment_reply_link(array_merge($args, array(
						...
						'reply_text' => '<span class="fa fa-reply"></span> ' . __('Reply', 'bootstrap-angular4'),
						'login_text' => '<span class="fa fa-reply"></span> ' . __('Log in to Reply', 'bootstrap-angular4')
					)));
					// end reply link
...
if (!function_exists('bootstrapBasicEditPostLink')) {
	...
	function bootstrapBasicEditPostLink() 
	{
		...
		if ($edit_post_link != null) {
		    $edit_btn = '<a class="post-edit-link btn btn-default btn-xs" href="'.$edit_post_link.'" title="' . __('Edit', 'bootstrap-angular4') . '"><i class="edit-post-icon glyphicon glyphicon-pencil" title="' . __('Edit', 'bootstrap-angular4') . '"></i></a>';
...
if (!function_exists('bootstrapBasicFullPageSearchForm')) {
	...
	function bootstrapBasicFullPageSearchForm() 
	{
		...
		$output .= '<input type="text" name="s" value="' . esc_attr(get_search_query()) . '" placeholder="' . esc_attr_x('Search &hellip;', 'placeholder', 'bootstrap-angular4') . '" title="' . esc_attr_x('Search &hellip;', 'label', 'bootstrap-angular4') . '" class="form-control" />';
		$output .= '</div>';
		$output .= '<div class="col-xs-2">';
		$output .= '<button type="submit" class="btn btn-default">' . __('Search', 'bootstrap-angular4') . '</button>';
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</form>';
...
if (!function_exists('bootstrapBasicMoreLinkText')) {
	...
	{
		return __('Continue reading <span class="meta-nav">&rarr;</span>', 'bootstrap-angular4');
...
if (!function_exists('bootstrapBasicPostOn')) {
	...
	function bootstrapBasicPostOn() 
	{
		...
		printf(__('<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'bootstrap-angular4'),
			...
			sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
				...
				esc_attr(sprintf(__('View all posts by %s', 'bootstrap-angular4'), get_the_author())),
...
if (!function_exists('bootstrapBasicTagsList')) {
	...
	{
		return sprintf('<span class="tags-icon glyphicon glyphicon-tags" title="' . __('Tagged', 'bootstrap-angular4') . '"></span>&nbsp; %1$s', $tags_list);
