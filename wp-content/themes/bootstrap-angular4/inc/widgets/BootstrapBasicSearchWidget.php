	...
	class BootstrapBasicSearchWidget extends WP_Widget
	{
		...
		public function __construct()
		{
			parent::__construct(
					...
					__('Bootstrap Navbar Search', 'bootstrap-angular4'), 
					array('description' => __('Display Search widget for Bootstrap navbar.', 'bootstrap-angular4'))
			);
		}// __construct
		...
		public function form($instance) 
		{
			...
			// output form
			$output = '<p>';
			$output .= '<label for="' . $this->get_field_id('navbaralign') . '">' . __('Form alignment:', 'bootstrap-angular4') . '</label>';
			$output .= '<select id="' . $this->get_field_id('navbaralign') . '" name="' . $this->get_field_name('navbaralign') . '">';
			$output .= '<option value="navbar-left"' . ($navbaralign == 'navbar-left' ? ' selected="selected"' : '') . '>' . __('Left', 'bootstrap-angular4') . '</option>';
			$output .= '<option value="navbar-right"' . ($navbaralign == 'navbar-right' ? ' selected="selected"' : '') . '>' . __('Right', 'bootstrap-angular4') . '</option>';
			$output .= '</select>';
			$output .= '</p>';
		...
		public function widget($args, $instance) 
		{
			...
			$output .= '<input type="text" name="s" class="form-control" placeholder="' . esc_attr_x('Search &hellip;', 'placeholder', 'bootstrap-angular4') . '" value="' . esc_attr(get_search_query()) . '" title="' . esc_attr_x('Search for:', 'label', 'bootstrap-angular4') . '">';
			$output .= '</div>';
			$output .= ' ';
			$output .= '<button type="submit" class="btn btn-default">' . esc_html__('Search', 'bootstrap-angular4') . '</button>';
			$output .= '</form>';
